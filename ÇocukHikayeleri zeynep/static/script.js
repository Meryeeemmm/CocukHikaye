...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const openModal = document.getElementById("openValuesModal");
  const closeModal = document.getElementById("closeValuesModal");
  ...
  closeModal.addEventListener("click", () => {
    ...
    // Seçilenleri butona yazdır
    const selected = Array.from(valuesForm.querySelectorAll("input[type='checkbox']:checked"))
      .map(cb => cb.value);
    
    if (selected.length > 0) {
      openModal.textContent = "🎈 " + selected.join(", ");
    } else {
      openModal.textContent = "🎈 Değerler";
    }
  });
  ...
  storyForm.addEventListener("submit", async (e) => {
    ...
    try {
      // Görsel için prompt başlıksız şekilde oluşturuluyor
      let imagePrompt = "";
      if (karakter) imagePrompt += `karakter: ${karakter}, `;
      if (gecisYeri) imagePrompt += `ortam: ${gecisYeri}, `;
      imagePrompt += "çocuk kitabı çizimi, canlı renkler, sevimli";
      ...
      imageResult.src = imageData.image_url;
      imageResult.alt = "Hikaye görseli";
      imageResult.style.display = "block";
      ...
      while (true) {
        ...
        if (done) break;
        const chunk = decoder.decode(value, { stream: true });
        storyContentP.textContent += chunk;
