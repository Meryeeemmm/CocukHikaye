...
#                 model="gpt-4o",
#                 messages=[
#                     {"role": "system", "content": "Sen selef  akidesini benimsemiş şirk ve tağuttan beri olmuş bir çocuk hikayesi yazma asistanısın.Hikaye kesinlikle belirtilen uzunlukta yaz."},
...
def generate_story():
    ...
    prompt += "\nLütfen hikaye içinde herhangi bir açıklama, yorum ya da '#' gibi işaretler kullanma. Sadece doğrudan hikayeyi yaz."


    if language == "en":
        prompt = (
            f"Create a moral and educational children's story in English for the age group '{yas_grubu}'. "
            f"The story should include a character named '{karakter}', take place in '{gecis_yeri}', and follow the genre '{hikaye_turu}'. "
            f"Story length should be '{uzunluk}'. Tags: {', '.join(degerler)}. Make it age-appropriate, simple, and educational."
        )
    else:
         prompt = (
 f"{yas_grubu} yaş grubu için ahlaki ve eğitici bir çocuk hikayesi yaz. "
            f"Hikayede '{karakter}' adlı bir karakter olsun, hikaye '{gecis_yeri}' yerinde geçsin ve hikaye türü '{hikaye_turu}' olsun. "
            f"Hikaye uzunluğu '{uzunluk}' kadar olsun. Etiketler: {', '.join(degerler)}. Hikaye çocuklara uygun, basit ve eğitici olsun."
        )
